...
`dm.pink_trombone_demo` (demo GUI; `pt.ui` is a deprecated compatibility copy).
...
`all.lib` registers the `pt` environment rather than flattening deprecated helper
names into its namespace, where `latch` would collide with `basics.lib`.
The old helper names and `pt.ui` remain available for at least one published release.
New code should use the documented core API and `dm.pink_trombone_demo`.
