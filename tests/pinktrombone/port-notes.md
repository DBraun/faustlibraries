...
helpers now have underscore names; documented deprecated aliases preserve the
old names for at least one published release. The canonical UI lives in
`dm.pink_trombone_demo`; `pt.ui` retains its previous implementation for compatibility.
The sequencer now uses the canonical demo, with the same automation paths.
